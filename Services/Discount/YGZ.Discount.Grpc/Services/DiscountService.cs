...
public class DiscountService : DiscountProtoService.DiscountProtoServiceBase
{
    ...
    public DiscountService(IDiscountRepository discountRepository, IMapper mapper, ISender sender)
    {
    ...
    public override async Task<BooleanResponse> CreateDiscount(CreateDiscountRequest request, ServerCallContext context)
    {
        ...
        var code = !string.IsNullOrEmpty(request.CouponModel.Code) ? Code.Of(request.CouponModel.Code) : Code.Create();
        var discountValue = request.CouponModel.DiscountValue ?? 0;
