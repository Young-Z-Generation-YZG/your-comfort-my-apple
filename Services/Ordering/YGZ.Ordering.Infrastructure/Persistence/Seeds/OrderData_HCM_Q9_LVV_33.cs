...
public static class OrderData_HCM_Q9_LVV_33
{
    ...
    public static IEnumerable<Order> Orders
    {
        get
        {
            ...
            // HCM_Q9_LVV_33: Prefers budget/value models (iPhone 15, iPhone 16E) with lower storage options
            var allModels = new List<(List<string> models, List<string> colors, string modelId, double weight)>
            {
                (iPhone15ModelsList, iPhone15ColorsList, ListModelIds.ModelIds["IPHONE_15"], 0.30), // 30% - preferred budget option
                (iPhone16ModelsList, iPhone16ColorsList, ListModelIds.ModelIds["IPHONE_16"], 0.20), // 20%
                (iPhone16EModelsList, iPhone16EColorsList, ListModelIds.ModelIds["IPHONE_16E"], 0.30), // 30% - preferred value option
                (iPhone17ModelsList, iPhone17ColorsList, ListModelIds.ModelIds["IPHONE_17"], 0.15), // 15%
                (iPhone17ProModelsList, iPhone17ProColorsList, ListModelIds.ModelIds["IPHONE_17_PRO"], 0.03), // 3% - rarely premium
                (iPhone17AirModelsList, iPhone17AirColorsList, ListModelIds.ModelIds["IPHONE_17_AIR"], 0.02) // 2% - rarely premium
            };

            var orders = new List<Order>();
            var random = new Random(3333); // Different seed for different pattern
            ...
            for (int i = 1; i <= 298; i++)
            {
                var datetimeKey = $"HCM_Q9_LVV_33_{i}_2025";
                var orderId = OrderId.Create();
                var orderItems = new List<OrderItem>();
                
                // Vary order size: 80% single item, 18% two items, 2% three items (more single-item orders)
                ...
                for (int j = 0; j < orderSize; j++)
                {
                    ...
                    // Prefer lower storage (128GB, 256GB) - 75% chance for budget-conscious customers
                    var storageRoll = random.NextDouble();
                    var storage = storageRoll switch
                    {
                        < 0.50 => StoragesList[0], // 50% - 128GB
                        < 0.75 => StoragesList[1], // 25% - 256GB
                        < 0.90 => StoragesList[2], // 15% - 512GB
                        _ => StoragesList[3] // 10% - 1TB
                    };

                    // Vary quantity: 92% single, 7% double, 1% triple (mostly single purchases)
                    ...
                    var orderItem = OrderItem.Create(
                        ...
                        tenantId: TenantId.Of(listTenantId[3]), // HCM_Q9_LVV_33
                        branchId: BranchId.Of(listBranchIds[3]), // HCM_Q9_LVV_33
                        orderId: orderId,
                        skuId: "SEED_DATA",
