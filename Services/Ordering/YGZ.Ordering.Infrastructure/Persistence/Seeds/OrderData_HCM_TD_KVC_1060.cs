...
public static class OrderData_HCM_TD_KVC_1060
{
    ...
    public static IEnumerable<Order> Orders
    {
        get
        {
            ...
            // HCM_TD_KVC_1060: Prefers premium models (iPhone 17 Pro/Air) with higher storage options
            var allModels = new List<(List<string> models, List<string> colors, string modelId, double weight)>
            {
                (iPhone15ModelsList, iPhone15ColorsList, ListModelIds.ModelIds["IPHONE_15"], 0.10), // 10% - budget option
                (iPhone16ModelsList, iPhone16ColorsList, ListModelIds.ModelIds["IPHONE_16"], 0.20), // 20%
                (iPhone16EModelsList, iPhone16EColorsList, ListModelIds.ModelIds["IPHONE_16E"], 0.15), // 15%
                (iPhone17ModelsList, iPhone17ColorsList, ListModelIds.ModelIds["IPHONE_17"], 0.20), // 20%
                (iPhone17ProModelsList, iPhone17ProColorsList, ListModelIds.ModelIds["IPHONE_17_PRO"], 0.25), // 25% - premium preference
                (iPhone17AirModelsList, iPhone17AirColorsList, ListModelIds.ModelIds["IPHONE_17_AIR"], 0.10) // 10%
            };

            var orders = new List<Order>();
            var random = new Random(1060); // Seed based on tenant identifier for consistency
            ...
            for (int i = 1; i <= 298; i++)
            {
                ...
                // Vary order size: 70% single item, 25% two items, 5% three items
                ...
                for (int j = 0; j < orderSize; j++)
                {
                    ...
                    // Prefer higher storage (256GB, 512GB, 1TB) - 80% chance
                    var storage = random.NextDouble() < 0.80
                        ? StoragesList.Skip(1).ElementAt(random.Next(StoragesList.Count - 1)) // Skip 128GB
                        : StoragesList[random.Next(StoragesList.Count)];

                    // Vary quantity: 85% single, 12% double, 3% triple
