...
public class GetPopularProductsHandler : IQueryHandler<GetPopularProductsQuery, PaginationResponse<PopularProductResponse>>
{
    ...
    public GetPopularProductsHandler(ILogger<GetPopularProductsHandler> logger, IMongoRepository<ProductModel, ModelId> modelRepository)
    {
        _logger = logger;
        _modelRepository = modelRepository;
    }

    public async Task<Result<PaginationResponse<PopularProductResponse>>> Handle(GetPopularProductsQuery request, CancellationToken cancellationToken)
    {
        // Default pagination: page 1, limit 5
        var page = 1;
        var limit = 5;

        // Sort by overall_sold descending (most popular first)
    ...
    private static PaginationResponse<PopularProductResponse> MapToResponse((List<ProductModel> items, int totalRecords, int totalPages) result, int page, int limit)
    {
        var items = result.items.Select(model => new PopularProductResponse
        {
            Id = model.Id.Value!,
        ...
        return new PaginationResponse<PopularProductResponse>
        {
