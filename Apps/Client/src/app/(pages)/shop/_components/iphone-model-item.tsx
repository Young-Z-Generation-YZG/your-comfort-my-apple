...
type IphoneModelItemProps = {
   modelItem: IIphoneModelResponse;
   title: string[];
   colorsHex: ColorProps[];
   storage: string[];
   averageRating: {
      rating: number;
      count: number;
   ...
   return (
      <Fragment>
         <div className="flex flex-row bg-white px-5 py-5 rounded-md">
            {/* image */}
            <div className="image flex basis-[20%] items-center justify-center h-[250px] rounded-lg overflow-hidden">
               <CldImage
                  width={1000}
                  height={1000}
                  className="w-auto h-[170%] object-cover"
                  src={`${modelItem.model_images[0].image_url}`}
                  alt="iPhone Model"
               />
            </div>
                     ...
                     {promotion ? (
                        <div>
                           ...
                           <p className="text-lg font-bold text-[#E11C48]">
                              ${promotion.minimum_promotion_price} - $
                              {promotion.maximum_promotion_price}
                           </p>
                        </div>
                     ) : (
                        <p
                           className={cn('text-sm font-normal', {
                              'text-base font-semibold': !promotion,
                           })}
                        >
                           {unitPriceRange
                              .map((price) => `$${price}`)
