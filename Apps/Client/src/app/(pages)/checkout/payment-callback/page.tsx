...
import { useIpnCallbackAsyncMutation } from '~/infrastructure/services/order.service';
...
const PaymentCallbackPage = () => {
   ...
   const hasRun = useRef(false); // Add this to track execution
   ...
   const [ipnCallbackAsync, { isLoading, isSuccess, isError, error, data }] =
      useIpnCallbackAsyncMutation();

   const form = useForm<VnpayIpnFormType>({
   ...
   const handleIpnCallback = async (data: VnpayIpnFormType) => {
      console.log('IPN Callback Data:', data);

      const result = await ipnCallbackAsync({
   ...
   useEffect(() => {
      handleIpnCallback(form.getValues());
   }, []);
